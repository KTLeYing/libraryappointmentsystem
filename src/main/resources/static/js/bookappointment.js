var bookappointment={
		...
		URL:{
			appoint:function(bookId,studentId){
				return '/book/'+bookId+'/appoint?studentId='+studentId;
			},
			verify:function(){
				return '/book'+'/verify';
			}
		...
		validateStudent:function(studentId,password){
			...
			if(!studentId||!password){
				return "nothing";
			}else if(studentId.length == 0 ||isNaN(studentId)||password.length == 0 ||isNaN(password)){
				return "typerror";
			}else {
				...
				if(!studentId||!password){
					...
					$('#studentBtn').click(function (){
						...
						if(temp=="nothing"){
							...
							$.cookie('studentId', studentId, {  path: '/book'});
							$.cookie('password', password, {  path: '/book'});
							// 跳转到预约逻辑 
